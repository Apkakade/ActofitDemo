...
class listOfAddressViewController: UIViewController , UITableViewDelegate,UITableViewDataSource{
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        lbl_back.text = langKeyWordForStaticValue.back
        lbl_navTitle.text = langKeyWordForStaticValue.AddAddress
        lbl_SaveAddress.text = langKeyWordForStaticValue.SaveAddress
        lbl_UsingGps.text = langKeyWordForStaticValue.UsingGPS
        lbl_currentLoc.text=langKeyWordForStaticValue.CurrentLocation
             
         if  SelectAddressFromVC == "CartScreenVC"
         {
            self.lbl_navTitle.text = "Select Address"
         }else{
           
            self.lbl_navTitle.text = "Add Address"
        }
        getAddressLocation()
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if SelectAddressFromVC == "CartScreenVC"
                {
                    objLocation = self.modelGetAddress[indexPath.row]
                    self.ShowAlert(langKeyWordForStaticValue.appName, AlertMessage: langKeyWordForStaticValue.confirmOrderMsg, AlertTag: 1)
